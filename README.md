<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=230&color=0:00E5FF,100:6A00FF&text=Hey,+I'm+Bibin+(bbinxx)!&fontColor=ffffff&fontSize=40&animation=fadeIn&fontAlignY=40"/>

### 🚀 Full-Stack Developer • Mobile App Developer  
B.Tech CSE Student | Loves building, designing, and breaking things for learning.

</div>

---

## 😎 Who Am I?

- B.Tech CSE student who codes like it's a survival skill  
- Backend with **Node.js + Express**, frontend with **Next.js**  
- Flutter developer who enjoys making smooth UIs & painful bugs disappear  
- I also do graphic design because pixels deserve respect too  
- Into Custom ROMs — yes, I flash phones like normal people change wallpapers  

> “If it suddenly works without reasons… I don’t touch it. That’s sacred code.”

---

## 🔧 Tech Stack

**Languages**  
<p><img src="https://skillicons.dev/icons?i=js,html,css,c,php,java,python" /></p>

**Frontend / Mobile**  
<p><img src="https://skillicons.dev/icons?i=react,nextjs,flutter,androidstudio" /></p>

**Backend & APIs**  
<p><img src="https://skillicons.dev/icons?i=nodejs,express" /></p>

**Databases**  
<p><img src="https://skillicons.dev/icons?i=mongodb,mysql,sqlite,firebase,supabase" /></p>

**Deployment & Cloud**
<p><img src="https://skillicons.dev/icons?i=vercel,netlify,render,digitalocean,cloudflare,heroku" /></p>

**Tools & Platforms**
<p><img src="https://skillicons.dev/icons?i=github,postman,linux,windows,canva" /></p>

**Design / Editing**
<p><img src="https://skillicons.dev/icons?i=ps,ai,pr,ae,blender" /></p>

**Also Used**
<p>Firebase Studio • Lovable • V0</p>

---

## 📚 Currently Learning

- Building cleaner backend architecture so future-me stops crying  
- Flutter animations that feel modern, not like a PowerPoint transition  
- MongoDB indexing — because `.find()` is not a life solution  
- Deployments that don’t activate “why is production on fire” mode  

> Learning progress: 60% docs, 30% debugging, 10% motivational music

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=bbinxx&theme=tokyonight&hide_border=true&count_private=true" height="150" />

<br/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=bbinxx&layout=compact&theme=tokyonight&hide_border=true" height="150" />

</div>

---

## 🎧 Currently Listening To
<div align="center">
  <img src="https://spotify-recently-played-readme.vercel.app/api?user=bnsojhyhhkabl2sn11yve8l3r&count=5">
</div>

> Music doesn’t fix bugs, but it makes the suffering stylish.

---

## 🤝 Connect With Me

- **LinkedIn:** https://www.linkedin.com/in/bibinraju/  
- **Email:** bibinraju541@gmail.com  
- **Instagram:** https://instagram.com/ibxbin  
- **GitHub:** https://github.com/bbinxx  

> If I don’t reply, I’m either coding or rebooting my brain.

---

<div align="center">

⭐ **If something here impressed you, confused you, or made you smile — drop a star.**  
It's the GitHub version of a high-five 😄

</div>
